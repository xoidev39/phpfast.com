...
class ComicController extends BaseController {
    ...
    public function __construct()
    {
        ...
        $this->modelComic = new PostsModel('comic',  LANG);
        ...
        if (!$this->postsModel->checkPosttypeExists()) {
            ...
            if(!empty($posts_latest)) {
                $modules[] = [
                    ...
                    'posttype' => $posttype,
                    'label' => ucfirst(Flang::_e('comic_new')),
            ...
            if(!empty($posts_popular)) {
                $modules[] = [
                    'type' => 'itemshorizontal',
                    'posttype' => $posttype,
                    'label' => ucfirst(Flang::_e('comic_hot')),
            ...
            if(!empty($posts_view_top)) {
                $modules[] = [
                    'type' => 'itemshorizontal',
                    'posttype' => $posttype,
                    'label' => ucfirst(Flang::_e('comic_view')),
    ...
    public function page($slug, $paged = 1) {
        ...
        $modules = [];
        $posts = $this->modelComic->ListsFields($fields,  $where, $params, $orderby, $paged, $limit);
        if(!empty($posts['data'])) { 
    ...
    public function detail($slug) {
        ...
        if (!empty($cachedata)) {
            ...
            $detail = $this->modelComic->getBySlug($slug);
            if(empty($detail)) {
                echo 'post not found';
                return false;
            }
            ...
            if(!empty($main_terms)) {
                ...
                if(!empty($post_ids)) {
                    ...
                    $related_post = $this->modelComic->ListsFields($fields, $where, $params, 'views DESC', $page, $limit);
            ...
            if($detail['comic_type'] == 'comic') {
                $this->assets->add('js', 'artplayer.js', 'footer');
                $this->data('assets_footer', $this->assets->footer('frontend'));
                $data_banner['single_comic'] = true;
                $data_banner['chapters'] = $detail['chapters'][0];
            } else {
                $data_banner['single_comic'] = false;
    ...
    private function _formatPosts($posts, $termshow = false, $posttype = '',  $relationshipShow = false) {
        ...
        foreach ($posts as $post) {
            ...
            if (!empty($post['comic_type'])){
                $postTmp['comic_type'] = $post['comic_type'];
            }
